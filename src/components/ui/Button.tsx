...
interface ButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  variant?: 'default' | 'outline' | 'secondary' | 'ghost' | 'destructive' | 'primary' | 'danger';
  size?: 'xs' | 'sm' | 'md' | 'lg';
  loading?: boolean;
  iconName?: string;
  iconSize?: number;
...
function Button({
  ...
  const sizes = {
    xs: 'h-7 rounded px-2 skote-small-text',
    sm: 'h-8 rounded-md px-3 skote-body-text',
    md: 'h-9 px-4 py-2 skote-body-text',
    lg: 'h-10 rounded-md px-8 skote-body-text',
  };
